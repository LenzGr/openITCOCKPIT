...
class MonitoringHelper extends AppHelper{
	...
	/**
	 * Check if ther is a difference betwen monitoring hoststatus flap_detection_ebabled and the itcockpit database configuration
	 * If yes it will return the current setting from $hostatus
	 * This can hapen, if a user disable the flep detection with an external command, but not in the host configuration
	 *
	 * @param array $host from find('first')
	 * @param array $hoststatus, if not given the $hoststatus array of the current view will be used (default)
	 * @return array with the flap detection settings. Array keys: 'string', 'html' and 'value'
	...
	public function compareHostFlapDetectionWithMonitoring($host, $hoststatus = null){
		...
		if($flapDetectionEnabledFromConfig != $flapDetectionEnabledFromMonitoring){
			...
			if($flapDetectionEnabledFromMonitoring == 1){
				return array('string' => __('Temporary on'), 'html' => '<a data-original-title="'.__('Difference to configuration detected').'" data-placement="bottom" rel="tooltip" href="javascript:void(0);"><i class="fa fa-exclamation-triangle txt-color-orange"></i></a> <span class="label bg-color-greenLight">'.__('Temporary on').'</span>', 'value' => $flapDetectionEnabledFromMonitoring);
			}
			return array('string' => __('Temporary off'), 'html' => '<a data-original-title="'.__('Difference to configuration detected').'" data-placement="bottom" rel="tooltip" href="javascript:void(0);"><i class="fa fa-exclamation-triangle txt-color-orange"></i></a> <span class="label bg-color-redLight">'.__('Temporary off').'</span>', 'value' => $flapDetectionEnabledFromMonitoring);
		}

		if($flapDetectionEnabledFromConfig == 1){
			return array('string' => __('On'), 'html' => '<span class="label bg-color-green">'.__('On').'</span>', 'value' => $flapDetectionEnabledFromConfig);
		}

		return array('string' => __('Off'), 'html' => '<span class="label bg-color-red">'.__('Off').'</span>', 'value' => $flapDetectionEnabledFromConfig);
	}

	/**
	 * Check if ther is a difference betwen monitoring serviestatus flap_detection_ebabled and the itcockpit database configuration
	 * If yes it will return the current setting from $servicestatus
	 * This can hapen, if a user disable the flep detection with an external command, but not in the host configuration
	 *
	 ...
	 * @param integer $notification_type from notifications (0 = host, 1 = service) [yes, nagios change this on EVERY table -.-]
